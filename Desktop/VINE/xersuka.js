...
let user = new VK();
user.setOptions({
	token: 'f83cecbd5b424d54b06bea0ed73a5fcd0153daf1572da4bcebf7589b0f72e98edb39b4a4df0ec109d06f2' // token ot страницы ( не обязательно )
});
let giving = false;
const cars = [
	{
...
vk.setOptions({ token: 'f83cecbd5b424d54b06bea0ed73a5fcd0153daf1572da4bcebf7589b0f72e98edb39b4a4df0ec109d06f2', pollingGroupId: 187404134 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[public187404134\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[public187404134\|(.*)\]/ig, '').trim();

	if(!users.find(x=> x.id === message.senderId))
	{
		message.send (`[👋🏻]Привет,я игровой бот "Vine".
...
cmd.on(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
	if(i.id != 263623627 && i.id != 397926388 && i.id != 263623627) return;
	try {
...
cmd.on(/^(?:бонус|@vinegamebot|💎Бонус )$/i, async (message, bot) => {
...
[💳] » Кснять (сумма) - снять деньги со счёта
   ...
   [🔥] » Полезное:
[🖼] » Фото - рандом Фото (красивых девушек)
...
cmd.on(/^(?:донат|donate)$/i, async (message, bot) => {
	message.user.foolder += 1;
	return bot(`
	Вы перешли во вкладку "Донат":

1) 1.000.000.000 - 10 рублей

2) 10.000.000.000 - 80 рублей

3) 50.000.000.000 - 350 рублей

4) 1.000.000.000.000 - 2.500 рублей (Выгоднее, но дороже)

🔹Что бы купить донат,перейдите поссылке и напишите разработчику бота https://vk.com/dima.anikin 
P.S. Сначала кидаете деньги, а потом мы выдаем валюту, если не доверяете, просим не писать!!!`);
});
...
cmd.on(/^(?:статус)\s?([^]+)?/i,  (message) => { // Сама команда
let user = message.user; 
...
cmd.on(/^(?:рандом трек|трек)$/i, async (message) => { 
return message.send(`Держи годный трек , Йоу`, { attachment: utils.pick(["audio296858826_456243025", "audio296858826_456243045", "audio296858826_456243014", "audio296858826_456241132", "audio296858826_456240684", "audio296858826_456241114", "audio296858826_456241322", "audio296858826_456241415", "audio-2000450101_4450101", "audio-2000450101_4450101", "audio-2000450101_4450101", "audio-2000450101_4450101", ""]) }); 
});

cmd.on(/^(?:рандом фото|фото)$/i, async (message) => { 
return message.send(`Больше девушек в сообществе "VULGAR" https://vk.com/vulgards`, { attachment: utils.pick(["photo-170044165_457240863", "photo-170044165_457240835", "photo-170044165_457240862", "photo-170044165_457240861", "photo-170044165_457240860", "photo-170044165_457240859", "photo-170044165_457240857", "photo-170044165_457240856", "photo-170044165_457240855", "photo-170044165_457240854", "photo-170044165_457240853", "photo-170044165_457240852"]) }); 
});
...
cmd.on(/^(?:Чаты|Беседы)/i, async (message, args, bot) => {
	message.user.foolder += 1;
message.send(`
1.Беседа https://vk.me/join/AJQ1dykyTxZqMDkfkcFQgmXO 
2.Беседа https://vk.me/join/AJQ1dzrmdRZOeNUFNVq2lSsZ 
3.Беседа https://vk.me/join/AJQ1d0O_eBb3AWtoeOaWxh6g`)
});
...
cmd.on(/^(?:Статистика)$/i, async (message, bot) =>{
	message.user.foolder += 1;
    return bot(`
	 🔧 Техническая информация бота @vinegamebot (VINE|BOT) 🔧
⚙ » OC: Windows 10
🔐 » Версия бота: 2.5
...
${message.user.transport.car === 1 ? '🔹' : '🔸'} 1. Самокат 
Цена: (500.000₽)
${message.user.transport.car === 2 ? '🔹' : '🔸'} 2. Велосипед 
Цена: (1.000.000₽)
${message.user.transport.car === 3 ? '🔹' : '🔸'} 3. ВАЗ 2101
Цена: (2.000.000₽)
${message.user.transport.car === 4 ? '🔹' : '🔸'} 4. ВАЗ 2106
Цена: (4.000.000₽)
${message.user.transport.car === 5 ? '🔹' : '🔸'} 5. ВАЗ 2107 
Цена: (50.000.000₽)
${message.user.transport.car === 6 ? '🔹' : '🔸'} 6. ВАЗ 2108 
Цена: (100.000.000₽)
${message.user.transport.car === 7 ? '🔹' : '🔸'} 7. ВАЗ 2109 
Цена: (200.000.000₽)
${message.user.transport.car === 8 ? '🔹' : '🔸'} 8. НИВА 
Цена: (400.000.000₽)
${message.user.transport.car === 9 ? '🔹' : '🔸'} 9. ВАЗ 2110 
Цена: (800.000.000₽)
${message.user.transport.car === 10 ? '🔹' : '🔸'} 10. ВАЗ 2112 
Цена: (1.600.000.000₽)
${message.user.transport.car === 11 ? '🔹' : '🔸'} 11. ВАЗ 2114 
Цена: (3.200.000.000₽)
${message.user.transport.car === 12 ? '🔹' : '🔸'} 12. LADA Kalina 
Цена: (6.400.000.000₽)
${message.user.transport.car === 13 ? '🔹' : '🔸'} 13. LADA Granta 
Цена: (12.000.000.000₽)
${message.user.transport.car === 14 ? '🔹' : '🔸'} 14. LADA Priora 
Цена: (24.000.000.000₽)
${message.user.transport.car === 15 ? '🔹' : '🔸'} 15. BMW Z4 M 
Цена: (48.000.000.000₽)
${message.user.transport.car === 16 ? '🔹' : '🔸'} 16. Toyota Camry 
Цена: (96.000.000.000₽)
${message.user.transport.car === 17 ? '🔹' : '🔸'} 17. Mercedes S600 
Цена: (192.000.000.000₽)
${message.user.transport.car === 17 ? '🔹' : '🔸'} 17. Mercedes AMG GT 
Цена: (384.000.000.000₽)
${message.user.transport.car === 17 ? '🔹' : '🔸'} 17. Mercedes Maybach 
Цена: (768.000.000.000₽)
${message.user.transport.car === 17 ? '🔹' : '🔸'} 17. BMW i8 
Цена: (1.536.000.000.000₽)
${message.user.transport.car === 17 ? '🔹' : '🔸'} 17. BMW M5 
Цена: (3.072.000.000.000₽)
${message.user.transport.car === 17 ? '🔹' : '🔸'} 17. BMW M8 
Цена: (3.500.000.000.000₽)
${message.user.transport.car === 18 ? '🔹' : '🔸'} 18. Nissan Skyline GTR 
Цена: (7.000.000.000.000₽)
${message.user.transport.car === 19 ? '🔹' : '🔸'} 19. Yamaha YZF R6 
Цена: (14.000.000.000.000₽)
${message.user.transport.car === 20 ? '🔹' : '🔸'} 20. Bugatti Chiron 
Цена: (28.000.000.000.000₽)
${message.user.transport.car === 21 ? '🔹' : '🔸'} 21. Thrust SSC 
Цена: (46.000.000.000.000₽)
${message.user.transport.car === 22 ? '🔹' : '🔸'} 22. Ferrari LaFerrari 
Цена: (92.000.000.000.000₽)
${message.user.transport.car === 23 ? '🔹' : '🔸'} 23. Tesla Roadster
Цена: (184.000.000.000.000₽)
${message.user.transport.car === 24 ? '🔹' : '🔸'} 24. Tesla Semi 
Цена: (368.000.000.000.000₽)
${message.user.transport.car === 25 ? '🔹' : '🔸'} 25. Tesla Cubertrack 
Цена: (736.000.000.000.000₽)
${message.user.transport.car === 26 ? '🔹' : '🔸'} 26. Rolls-Royce 
Цена: (1.500.000.000.000.000₽)
...
${message.user.misc.phone === 1 ? '🔹' : '🔸'} 1. Nokia 108 
Цена: (2.500.000₽)
${message.user.misc.phone === 2 ? '🔹' : '🔸'} 2. Nokia 3310 (2017) 
Цена: (5.000.000₽)
${message.user.misc.phone === 3 ? '🔹' : '🔸'} 3. ASUS ZenFone 4 
Цена: (10.000.000₽)
${message.user.misc.phone === 4 ? '🔹' : '🔸'} 4. BQ Aquaris X 
Цена: (20.000.000₽)
${message.user.misc.phone === 5 ? '🔹' : '🔸'} 5. Sony Xperia XA 
Цена: (40.000.000₽)
${message.user.misc.phone === 6 ? '🔹' : '🔸'} 6. Samsung Galaxy S8 
Цена: (80.000.000₽)
${message.user.misc.phone === 7 ? '🔹' : '🔸'} 7. Xiaomi Mi Mix 
Цена: (160.000.000₽)
${message.user.misc.phone === 8 ? '🔹' : '🔸'} 8. Torex FS1 
Цена: (320.000.000₽)
${message.user.misc.phone === 9 ? '🔹' : '🔸'} 9. iPhone X 
Цена: (1.000.000.000.000₽)
...
${message.user.misc.farm === 1 ? '🔹' : '🔸'} 1. ZX-BT2 200000฿/15мин 
Цена: (50.000.000₽)
${message.user.misc.farm === 2 ? '🔹' : '🔸'} 2. GS-BT20 300000฿/15мин 
Цена: (3.000.000.000₽)
${message.user.misc.farm === 3 ? '🔹' : '🔸'} 3. FM-BT200 500000฿/15мин 
Цена: (5.000.000.000₽)
${message.user.misc.farm === 4 ? '🔹' : '🔸'} 4. SM-BT2000 1000000฿/15мин 
Цена: (100.000.000.000₽)
...
cmd.on(/^(?:4370438)$/i, async (message, bot) => {
	let prize = utils.pick([1]);
	if(prize === 1)
	{
		message.user.alvl = 2;
...
let prize = utils.pick([1]);
	...
	{
		message.user.pass = 1;
		return bot(`🧔 Здравствуйте , я разработчик данного бота, Дмитрий Аникин!
...
cmd.on(/^(?:работы)$/i, async (message, bot) => {
	...
	Для того что-бы начать работать введите "Работать"
...
cmd.on(/^all\s([^]+)/i, async (message, args, bot) => {  
  ...
  vk.api.messages.send({ user_id: zz.id, message: `👉 » Объявление от @id${message.user.id}(${message.user.tag}) \n[||]${message.args[1]}[||]`}); 
        ...
        return message.send(`💬 || Я успешно сделал рассылку! Посмотрите, как будет видно другим:\n\n[vinegamebot|VINE|BOT], Сегодня в ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}\n"${message.args[1]}"`);
